...
bool sequenceRepeatCheck(long long int num) {
    string sNum = to_string(num);
    long long int numLength = sNum.length();

    for (long long int len = 1; len <= numLength / 2; len++) {
        string sequence = sNum.substr(0, len);
        bool repeat = true;

        for (long long int i = 0; i < numLength; i += len) {

            if (sNum.substr(i, len) != sequence) {
                repeat = false;
                break;
            }
        }
    ...
    while (getline (fileRead, fileLine)) {
        ...
        for (int i = 0; i < vRanges.size(); i++) {
            ...
            for (long long int j = startPoint; j <= endPoint; ++j) {
                repeatFound = sequenceRepeatCheck(j);
